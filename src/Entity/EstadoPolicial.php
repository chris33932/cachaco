...
/**
 * @ORM\Entity(repositoryClass=EstadoPolicialRepository::class)
 */
class EstadoPolicial
{
    /**
     * @ORM\Id
     * @ORM\GeneratedValue
    ...
    /**
     * @ORM\OneToMany(targetEntity=Victima::class, mappedBy="est_pol")
     */
    private $victimas;

    /**
     * @ORM\OneToMany(targetEntity=PresAutor::class, mappedBy="est_pol")
     */
    private $presAutors;
    ...
    public function removeVictima(Victima $victima): self
    {
        if ($this->victimas->removeElement($victima)) {
            ...
            if ($victima->getEstPol() === $this) {
                $victima->setEstPol(null);
            }
        }
    ...
    public function removePresAutor(PresAutor $presAutor): self
    {
        if ($this->presAutors->removeElement($presAutor)) {
            // set the owning side to null (unless already changed)
            if ($presAutor->getEstPol() === $this) {
                $presAutor->setEstPol(null);
            }
        }
