...
class RangoEtario
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=PresAutor::class, mappedBy="rango_etario")
     */
    private $presAutors;
    ...
    public function removePresAutor(PresAutor $presAutor): self
    {
        if ($this->presAutors->removeElement($presAutor)) {
            // set the owning side to null (unless already changed)
            if ($presAutor->getRangoEtario() === $this) {
                $presAutor->setRangoEtario(null);
            }
        }
